...
public static class Program
{
    ...
    private static void RenameDirectories(string currentDirectory, string originalName, string newName)
    {
        ...
        foreach (var directory in directories)
        {
            ...
            if (newDirectoryName != directory) Directory.Move(directory, newDirectoryName);
        }

        directories = Directory.GetDirectories(currentDirectory);
        foreach (var directory in directories) RenameDirectories(directory, originalName, newName);
    }
    ...
    private static void RenameFiles(string currentDirectory, string originalName, string newName)
    {
        ...
        foreach (var file in files)
        {
            ...
            if (newFileName != file) Directory.Move(file, newFileName);
        }

        var subDirectories = Directory.GetDirectories(currentDirectory);
        foreach (var directory in subDirectories) RenameFiles(directory, originalName, newName);
    }
    ...
    private static void RenameFileContents(string currentDirectory, string originalName, string newName)
    {
        ...
        foreach (var file in files)
        {
            if (file.EndsWith(".exe") || file.EndsWith(".dll") || file.EndsWith(".runtimeconfig.json")) continue;
        ...
        var subDirectories = Directory.GetDirectories(currentDirectory);
        foreach (var directory in subDirectories) RenameFileContents(directory, originalName, newName);
    }
}